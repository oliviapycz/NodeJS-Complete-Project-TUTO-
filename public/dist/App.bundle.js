...
function autocomplete(input, latInput, lngInput) {
    ...
    // if someone hits entoer on the address field, don't submit the form
...
function typeAhead(search) {
    ...
    // on is a shortcut for addEventListener
    searchInput.on('input', function () {
        ...
        _axios2.default.get('/api/' + process.env.VERSION + '/search?q=' + this.value).then(function (res) {
            if (res.data.length) {
                searchResults.innerHTML = _dompurify2.default.sanitize(searchResultsHTML(res.data));
                return;
            }
            // tell them nothing came back
            searchResultsHTML = _dompurify2.default.sanitize('<div class="search__result">No results for ' + _this.value + ' found</div>');
        }).catch(function (err) {
            console.error(err);
        });
    });
